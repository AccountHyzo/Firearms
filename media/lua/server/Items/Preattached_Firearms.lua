
WeaponUpgrades = {
	Shotgun		= {"Sling", "Sling_Leather", "ChokeTubeFull", "ChokeTubeImproved", "AmmoStraps", "RedDot", "AmmoStock", "RecoilPad", "x2Scope"},
	Mossberg500		= {"Sling", "ChokeTubeFull", "ChokeTubeImproved", "AmmoStraps", "RedDot", "AmmoStock", "RecoilPad", "x2Scope"},
	Remington870Wood		= {"Sling", "Sling_Leather", "ChokeTubeFull", "ChokeTubeImproved", "AmmoStraps", "RedDot", "AmmoStock", "RecoilPad", "x2Scope"},
  Mossberg500Tactical		= {"Sling", "ChokeTubeFull", "ChokeTubeImproved", "TacticalStock", "ShotgunStock", "LightShotgunStock", "RedDot", "AmmoStock", "RecoilPad", "x2Scope"},

  AssaultRifle		= {"Sling_Olive", "Sling_Camo", "AmmoStraps", "RedDot", "x2Scope", "x4Scope"},
  M16A2		= {"Sling_Olive", "Sling_Camo", "AmmoStraps", "RedDot", "x2Scope", "x4Scope"},

  MP5		= {"Sling", "RedDot", "x2Scope", "x4Scope", "9mmSilencer", "MP5_Stock_Detracted"},
  UZI    = {"9mmSilencer", "RedDot", "x2Scope", "Sling"},

  Glock17		= {"RedDot", "9mmCompensator", "Laser"},

  HuntingRifle		= {"Sling_Leather", "RecoilPad", "AmmoStraps", "8xScope"},
  VarmintRifle		= {"Sling", "RecoilPad", "AmmoStraps", "8xScope"},
  M24Rifle		= {"Sling_Olive", "Sling_Camo", "ExtendedRecoilPad", "Rifle_Bipod", "AmmoStraps", "8xScope", "x4-x12Scope", "308Silencer"},

};
